...
public void array8 (){
	...
	for(int outer_index = 0;outer_index < factura.length;outer_index++){
	System.out.println("Variable outer_index:"+outer_index);
		
		for (int midle_index = 0;midle_index < factura[outer_index].length;midle_index++){
			System.out.println("Variable midle_index:"+midle_index);
			
			for (int inner_index = 0;inner_index < factura[outer_index][midle_index].length;inner_index++){
				System.out.println("Variable inner_index:"+inner_index);	
				
				suma+= factura [outer_index][midle_index][inner_index];
				total++;
		}		
		}}
		System.out.println("El valor medio del array 3D es:"+suma/total);
	}

// String y uso de Char 
public void array9() throws IOException {
int ch = 0;
